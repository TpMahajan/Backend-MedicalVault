  ...
  try {
    ...
    if (!doctorId || !appointmentDate || !appointmentTime || !reason) {
      return res.status(400).json({
        success: false,
        message:
          "doctorId, appointmentDate, appointmentTime, and reason are required.",
      });
    }
    ...
    if (!isForSelf) {
      ...
      const linked =
        user?.linkedProfiles?.map((p) => p?.toString?.() || p) || [];
    ...
    if (doctor.fcmToken) {
      await sendNotificationToDoctor(
        ...
        } has requested an appointment for ${new Date(
          appointmentDate
...
router.get("/appointments/upcoming", async (req, res) => {
  try {
    ...
    const upcoming = appointments.filter((apt) => {
      ...
      const [h, m] = (apt.appointmentTime || "00:00")
        .split(":")
        .map(Number);
  ...
  try {
    ...
    const past = combined
      ...
      .sort((a, b) => {
        ...
        const [ha, ma] = (a.appointmentTime || "00:00")
          .split(":")
          .map(Number);
        da.setHours(ha, ma, 0, 0);
        const db = new Date(b.appointmentDate);
        const [hb, mb] = (b.appointmentTime || "00:00")
          .split(":")
          .map(Number);
...
router.post("/appointments/:id/reschedule-request", async (req, res) => {
  try {
    ...
    const [h, m] = (appointment.appointmentTime || "00:00")
      .split(":")
      .map(Number);
...
router.get("/appointments/:id/ai-summary", async (req, res) => {
  try {
    ...
    const insight = await AppointmentAIInsight.findOne({
      appointmentId: id,
    }).lean();
    ...
    if (!insight) {
      return res.json({
        ...
        message:
          "AI summary not yet generated. Use the AI assistant to ask about this visit.",
      });
    }
