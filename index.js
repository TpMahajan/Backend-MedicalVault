...
import appUpdateRoutes, {
  validateAppUpdateConfigOnStartup,
} from "./routes/appUpdate.js";
...
const envCorsOriginPatterns = (process.env.CORS_ORIGIN_PATTERNS || "")
  ...
  .flatMap((pattern) => {
    try {
      ...
      console.warn(
        `[cors] ignoring invalid CORS_ORIGIN_PATTERNS entry: ${pattern}`,
        error,
      );
      return [];
    }
...
const startServer = async () => {
  try {
    ...
    await validateAppUpdateConfigOnStartup();

    const sessionCleanupIntervalMs = Number(
      process.env.SESSION_HISTORY_FLUSH_INTERVAL_MS || 30000,
    );
